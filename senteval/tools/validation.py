...
class KFoldClassifier(object):
    ...
    def run(self, params):
        ...
        if self.config['adversarial_sample_generator'] is not None:
            ...
            adv_embed_x, adv_embed_y, adv_sentences = self.config['adversarial_sample_generator'](self.test['X'],
                                                                                                  self.test['y'],
                                                                                                  batcher=batcher)
...
class SplitClassifier(object):
    ...
    def run(self, params):
        ...
        label_diff_count = 0
        if self.config['adversarial_sample_generator'] is not None:
            adv_embed_x, adv_embed_y, adv_sentences = self.config['adversarial_sample_generator'](self.X['test'],
                                                                                                  self.y['test'])
            adv_preds = []
            for i in range(len(adv_embed_x)):
